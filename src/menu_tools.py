...
def SetSelectMenu(buttons, SPACE=10, COVERAGE=(80, 80), **kwargs):
    ...
    if def_kwargs["sz"][0] is None:
        to_put = (HEIGHT * COVERAGE[0] //
        (len(buttons) * 100 + (len(buttons) - 1) * SPACE))
    ...
    center = [(HEIGHT - (len(buttons) - 1) *
               def_kwargs["sz"][0] * (100 + SPACE) // 100) // 2, WIDTH // 2]
    for button in buttons:
        AddButton(**(def_kwargs | ({"center": center} | button)))
        center[0] += (def_kwargs["sz"][0] * (100 + SPACE)) // 100

GLOBAL_SURFACE = pygame.Surface((WIDTH, HEIGHT))
def ShowText(text, timer=None, **kwargs):
    ...
    for button in buttons_args:
        normal_size = NormalSizeOfButton(txt_fnt=button["txt_fnt"],
                                         txt_sz=button["txt_sz"],
                                         txt=button["txt"])
    ...
    corner_size = 5
    substrate_pos = (HEIGHT // 2 - req_space[0] // 2 - corner_size,
                     WIDTH // 2 - req_space[1] // 2 - corner_size)
    pygame.draw.rect(screen, WHITE[:],
                     (*substrate_pos[::-1],
                      req_space[1] + 2 * corner_size,
                      req_space[0] + 2 * corner_size))
    
    SetSelectMenu(buttons_args, SPACE=0,
                  sz=(req_space[0] // len(lines), req_space[1]), mode="BASIC")
    pygame.display.flip()
    time.sleep(timer)
